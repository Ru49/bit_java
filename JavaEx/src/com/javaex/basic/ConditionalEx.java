...
public class ConditionalEx {
	...
	public static void continueEx() {
		// 1부터 100까지 for loop
		// 2의 배수이거나 3의 배수며 출력 안함
		// 나머지느 출력
		for(int i =1; i <= 100; i++) {
			// 반복문 중간에 continue를 만나며 남은 구문은 실행하지 않고 다음번 루프 진행
			if(i % 2 == 0 || i % 3 == 0) {
	...
	public static void dowhileEx() {
		...
		// 아니면 반복 조건이 loop문 내부에서 결정되는 경우 do ~ while
		do { // 일다 실행
	...
	public static void whileEx3() {
		// while은 종료를 위한 제어 변수를 잘 제어하여 무한루프에 빠지지 않도록 해야한다
	...
	public static void switchEx() {
		...
		// switch는 전달받은 변수의 ㄱ밧을 이용해서 흐름을 제어 (==)
		Scanner scanner = new Scanner(System.in);
		System.out.print("1:C, 2:C++, 3:Java, 4:Python -");
		
		int code = scanner.nextInt();
		switch(code) { // switch는 변수의값을 비교 흐름 제어
		case 1: // code == 1
