...
//		stringMethods();
	...
	private static void stringMethods() {
		...
		// 검색 관련 : 문자, 문자열의 위치 찾을 떄
		System.out.println("5번 인덱스의 문자: " + s.charAt(5));
		
		// 중요! indexOf
		System.out.println("Fun 문자열의 위치 인덱스: " + s.indexOf("Fun")); // 검색어 대소문자를 구분
		System.out.println("fun 문자열의 위치 인데스: " + s.indexOf("fun")); // -1 -> 찾을 수 없음
		System.out.println("m의 위치의 인덱스: " + s.indexOf("m"));
		System.out.println("m의 역방향 위치 인덱스: " + s.lastIndexOf("m"));
 		
		System.out.println("m의 위치 인덱스 2nd: " + s.indexOf("m", 12)); // 검색 시작 위치를 12번 익데스부터
		...
		// 변환 메서드를 수행해도 내부 데이터는 변경되는 않는다(중요: immutable)
	...
	private static void stringBasic() {
		...
		String s2 = new String("Java"); // 메모리에 개 객체
		String s3 = "Java"; // 리터럴
