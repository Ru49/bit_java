...
public class VectorEx {
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i <= 10; i++) {
			v.addElement(i);
			// v.adElement(Interger.valueOf(i)):
		}
		v.addElement(11); // 하용량 초과 -> 버퍼 자동 증가
		System.out.printf("size: %d, capccit: %d%n", v.size(), v.capacity());
		
		// 객체의 삽입
		v.insertElementAt(5, 7); // 5 객체를 7번 인덱스에 추가
		System.out.println("vector:" + v);
		
		// 특정 인덱스에 있는 객체를 반환 :  elemnetAt
		...
		// 없는 객체의 검색 : -1 인덱스
		System.out.println("없는 객체의 인데스? " + v.indexOf(0));
		...
		v.clear(); // 내부 버퍼 지우기
		System.out.println("vector: " + v);
		System.out.printf("size: %d, capccit: %d%n", v.size(), v.capacity()); // 늘어난 버퍼는 유지
